...
// security-scoped URL). swap_file_dispose skips truncation for external
// fds (the file is on a user-chosen drive and may be reused), but always
// closes the descriptor -- this is the kernel's copy, not the caller's.
...
static void swap_file_dispose(int fd, uint64_t *bitmap, bool external) {
    if (fd >= 0) {
        // For external fds (e.g. USB swap), skip the truncate: the file is on
        // a user-chosen drive and the caller may want it preserved. The close
        // still happens -- this fd is the kernel's copy, not the caller's.
        if (!external) {
...
static int swap_enable_locked(uint64_t bytes, bool ram_only, int host_fd) {
    ...
    // creating an anonymous temp file. The fd is used as-is; the caller is
    // responsible for its lifecycle (close on swap_disable or error).
    int fd = -1;
    if (!ram_only) {
        if (host_fd >= 0)
            fd = host_fd;
    ...
    if (!ram_only) {
        ...
        if (err != 0) {
            // For external fds, do NOT close here: the caller opened it and
            // is responsible for closing on error. For internal fds (mkstemp),
            // close now since nobody else holds the descriptor.
            if (host_fd < 0)
                close(fd);
            free(bitmap);
...
void swap_startup(void) {
    if (atomic_load_explicit(&swap_pref_seen, memory_order_acquire)) {
        ...
        swap_external_fd = -1;  // consumed once
        int err;
        if (ext_fd >= 0) {
