...
if [ "$is_x86_guest" -eq 1 ]; then
    ...
    all_tests="rep_interruptible atomic_xadd32 atomic_cmpxchg32 atomic_cmpxchg8b atomic_logic32 atomic_lock_contended atomic_neg_not cow_atomic_fault x87_fpu x86_loop port_io_gpf cpuid_xsave cpuid_signature fpu_state_span x86_smc popf_ac_id null_deref_siginfo smc_patch_race x86_smc_alias x86_fp_env x86_ud_siginfo x86_prefetch $all_tests"
fi
if [ "$is_x86_guest" -eq 1 ] && [ "$is_amd64_guest" -eq 0 ]; then
