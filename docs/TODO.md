...
- **pwritev2 ignores its flags.** sys_pwritev2_guest (kernel/fs.c) takes `flags` as UNUSED. musl 1.2.6 (Alpine 3.24) calls every pwrite() as pwritev2(..., RWF_NOAPPEND), so a pwrite on an O_APPEND descriptor appends where Linux writes at the offset. Honour RWF_NOAPPEND (write at the offset), and return EOPNOTSUPP for flags AOK cannot do, which is what musl falls back on.
