...
- **O_PATH through a /proc link to a pipe, socket, anonymous inode or removed directory.** Linux opens all four (camd, 6.12): `open("/proc/self/fd/N", O_PATH)` of a pipe is an S_IFIFO handle on the pipe's inode, a socket S_IFSOCK, an eventfd its anon inode, and a directory removed while held still opens. AOK now gives O_PATH the held file for regular files and directories (557), but these have no name to reopen by and no filesystem reopen, so procfd_open_path (fs/generic.c) leaves the first three to the text walk (ENOENT) and refuses the fourth with ENOENT, since the walk could reach whatever took its name. The fix is an O_PATH handle that holds the description itself -- the opath pseudo-fd (opath_link_fd_create) with a retained struct fd and fstat forwarded -- plus a directory reopen in realfs and tmpfs.
- **F_GETFL of an ordinary descriptor: O_DIRECTORY and O_LARGEFILE.** On camd a directory reopened through /proc reports O_LARGEFILE|O_DIRECTORY (0x18000 on x86_64); AOK reports 0, because realfs_getflags asks the host, whose F_GETFL keeps neither. A 64-bit Linux reports O_LARGEFILE on every open. Measure the rule per ABI (-m32 differs) before changing fd_getflags.
...
- **Promote Alpine 3.24.2 from community to official.** Published 2026-09-26 as a community (experimental) download for all four architectures (ish-AOK-rootfs 1bf292d). On the Mac CLI, a 34-package install and workload sweep passed on x86, and a 13-check smoke test (apk, python3 with multiprocessing, git, curl, ssh) passed on x86, x86_64, aarch64 and riscv64. Still to do: move the alpine-i386-test (3.20.3), alpine-amd64-test (3.23.3) and alpine-riscv64-test (3.20.10) roots to 3.24, run the five-root suite on the Mac and on the device, then flip `tier` to official and drop "(Experimental)".
- **i386: misaligned 16- and 32-bit LOCK operations fault.** The i386 JIT's locked gadgets check alignment and then ignore the answer. The affected ops are cmpxchg, add/sub/adc/sbb/and/or/xor, inc/dec/not/neg, xadd, xchg and bts/btr/btc, in jit/gadgets-aarch64/misc.S, math.S and bits.S. On a misaligned operand they call only the no-op helper_trace_unaligned_atomic, then run ldaxr/stlxr on the misaligned host address anyway.
  - On an M-series Mac that works inside a 16-byte block and raises SIGBUS across one (`dword_at_14` in tests/manual/x86_unaligned_lock). Older devices may fault on any misalignment.
  - x86 allows it. The i386 ABI aligns 16- and 32-bit fields naturally, so only packed structures reach it; the ABI-driven 64-bit case (`cmpxchg8b`) is fixed.
