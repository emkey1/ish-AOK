...
- **Say " (deleted)" in /proc links to an unlinked file.** Linux's /proc/<pid>/fd/N and exe of a file unlinked while held read `/path (deleted)`; AOK's read the bare old name (seen again with the O_PATH work; exec_fd_pathless notes it as /proc's to fix). Tools test for the suffix to spot a replaced binary or a held deleted file (lsof, needrestart, systemd). generic_getpath_shown would add it when the name no longer reaches the file (path_names_file), as it already does for a memfd.
