...
| [#575](https://github.com/emkey1/ish-AOK/issues/575) | Unable to delete machines | **fixed 2026-09-26 for 557**: a machine deletes from the list by swiping its row, through the same confirmation as its Delete Filesystem row (which now names the root), and the in-use footers say how to free one (boot another first). Verified in a simulator. Still open on GitHub -- close it with the maintainer's OK |
