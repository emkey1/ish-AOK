...
## 2026-03-29 - Avoid strcmp overhead for "." and ".."
**Learning:** In VFS directory iteration loops (e.g., `migrate_host_names`), modern C compilers already optimize `strcmp` against small string literals (like `.` and `..`) into efficient inline integer comparisons. However, manual byte-comparisons `name[0] == '.' && ...` wrapped in a `static inline bool is_dot_or_dotdot` function avoid the function overhead of `strcmp` and perform measurably faster, reducing O(N) operations in hot VFS loops.
**Action:** Replace `strcmp(ent->d_name, ".") == 0 || strcmp(ent->d_name, "..") == 0` with manual byte-comparisons via a helper function `is_dot_or_dotdot` to avoid function overhead while preserving code readability.
