...
## 2026-03-29 - Optimize finding "." and ".." strings in tight loops
**Learning:** In VFS and FileProvider enumeration paths, checking if a string represents the current or parent directory `.` or `..` using `strcmp` in a `readdir` loop introduces significant O(N) traversal overhead (and function call overhead) that is completely unnecessary for fixed strings of 1 and 2 characters.
**Action:** Replace `strcmp(name, ".") == 0 || strcmp(name, "..") == 0` with a custom `static inline bool is_dot_or_dotdot(name)` helper that does manual character array indexing (`name[0] == '.' && name[1] == '\0'`). This effectively turns O(N) string comparisons into constant time O(1) checks.
