...
## 2026-09-14 - Redundant string check optimization via inline functions
**Learning:** `strcmp(x, "..")` is already fast (stops by the third byte) and compilers optimize it effectively, even eliminating the call entirely for small strings in loops at higher optimization levels (e.g. -O2 or -Os). Manually replacing `strcmp` with custom `inline` functions for string checks (like `is_dot_or_dotdot`) does not remove the underlying call or provide meaningful O(1) performance benefits, especially if the surrounding loop is dominated by much more expensive operations (like SQLite reads, object allocations, or `NSLog`). Furthermore, dynamically inserting identical copies of an inline helper into multiple files via scripts is error-prone, violates DRY principles, risks breaking platform-specific conditional compilation blocks (e.g., `#if defined(__linux__)`), and causes nasty merge conflicts (`redefinition of ...`).
**Action:** Do not micro-optimize `strcmp` calls against short static strings (`"."`, `".."`) using custom inline helpers. Compilers handle this efficiently. Do not attempt to dynamically inject C code blocks (like helper functions) into multiple files; if a shared helper is truly needed, put it in a common header like `misc.h`. Always consider the absolute cost of the loop body: micro-optimizing a string check is worthless if the loop performs disk I/O or logging.
