...
## 2026-03-29 - Optimize string literals in directory iteration loops
**Learning:** Performance optimization technique: In VFS directory iteration loops (e.g., `fakefs_readdir`, `scan_host_dir`), modern C compilers already optimize `strcmp` against small string literals (like `.` and `..`) into efficient inline integer comparisons. However, replacing these `strcmp` calls with explicit manual byte-comparisons (e.g. `is_dot_or_dotdot`) guarantees O(1) performance and avoids function overhead, which is highly beneficial in hot loops like VFS directory iterations where string literals are frequently checked.
**Action:** When working on VFS path matching or list traversals inside hot loops (like directory iteration), replace `strcmp(str, ".") == 0 || strcmp(str, "..") == 0` with a fast O(1) manual byte-comparison wrapped in a `static inline bool is_dot_or_dotdot(const char *name)` helper function to preserve code readability and pass code review while optimizing performance.

## 2026-03-30 - Avoid manual rewrites of compiler-optimized operations
**Learning:** An optimization that surprisingly DIDN'T work (and why): Replacing `strcmp(name, ".") == 0 || strcmp(name, "..") == 0` with a manual byte comparison `is_dot_or_dotdot` in `fs/fake-migrate.c` was rejected because `clang -O2` already compiles the `strcmp` logic into exact, highly-efficient byte comparisons without function call overhead. Additionally, `fs/fake-migrate.c` is only executed during one-time schema migration (`fakefs_migrate()`), not in hot VFS loops like `fakefs_readdir`, meaning there was no measurable performance bottleneck to optimize.
**Action:** Before optimizing seemingly inefficient code (like `strcmp` against string literals), verify if the compiler already optimizes it at the current optimization level (e.g. `clang -O2`). Furthermore, ensure the target code path is genuinely a hot loop or performance bottleneck, and not a cold path such as one-time initialization or migration logic. Avoid rewriting standard library calls with manual logic if it provides no actual measurable impact.
