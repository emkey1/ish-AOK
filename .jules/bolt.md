...
## 2026-03-30 - Avoid manual string comparisons for small literals and avoid optimizing cold paths
**Learning:** Modern compilers (like clang and GCC) already expand `strcmp(x, ".")` and `strcmp(x, "..")` against string literals into efficient inline byte comparisons, so there is no actual function call overhead to eliminate. Furthermore, micro-optimizing string comparisons in cold paths (like migration code in `fs/fake-migrate.c` which only runs at mount or schema upgrade) adds risk with no measurable gain.
**Action:** Do not manually replace `strcmp` against small string literals with hand-rolled character checks, and avoid premature micro-optimizations on cold paths.
