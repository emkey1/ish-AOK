...
## 2026-09-25 - Caching string lengths in arrays at compile time
**Learning:** In C, iterating over an array of statically known strings and calling `strlen` on them during runtime inside a loop (like in `kernel/fs.c` for `ldconfig_trace_path`) incurs unnecessary overhead. The `strlen` calculation can be done at compile time.
**Action:** Replace `const char *prefixes[]` containing string literals with an array of structs that store both the string and its compile-time evaluated length using `sizeof(literal) - 1`. This eliminates runtime `strlen` operations and replaces them with an O(1) constant, significantly speeding up hot paths like filesystem tracing loops.
