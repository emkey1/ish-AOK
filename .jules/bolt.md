...
## 2026-05-24 - Optimize string length calculations in VFS path construction
**Learning:** In iSH VFS path construction functions (e.g., `tmpfs_getpath`, `proc_getpath`), building a path string backwards from the end of a buffer and then passing the result to `memmove(buf, p, strlen(p) + 1)` causes an unnecessary O(N) recalculation of the string length. Since the buffer's upper boundary is fixed and the current pointer `p` is known, calculating length via pointer arithmetic is an O(1) operation.
**Action:** When building a path string backwards from the end of a buffer (`buf + MAX_PATH - 1`), calculate the length using pointer arithmetic (e.g., `(buf + MAX_PATH) - p`) instead of calling `strlen()` on the resulting pointer or maintaining manual size accumulators dynamically inside the loop. This eliminates redundant string traversals.
