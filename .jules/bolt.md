...
## 2026-03-30 - Identify hot paths for optimization
**Learning:** An optimization that replacing `strcmp` with manual byte comparison in `fs/fake-migrate.c` was rejected. This is because `fs/fake-migrate.c` is migration code and runs only once (at mount or schema upgrade), not in a hot VFS path. Additionally, modern compilers already optimize `strcmp` against string literals like `.` into inline byte comparisons, so manual optimizations here offer no measurable gain and introduce unnecessary risk in critical migration logic.
**Action:** Before optimizing a string operation or any perceived bottleneck, verify that the code actually resides on a hot path (e.g., runs per-operation in steady state, not just at startup/mount). Do not manually inline `strcmp` for short string literals unless profiling shows a specific compiler failure to optimize it on a hot path.
