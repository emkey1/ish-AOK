...
## 2024-05-29 - Cache redundant strlen results in consecutive calls
**Learning:** In `kernel/log.c`, `log_buf_append` called `strlen(msg)` three consecutive times for the same unchanged parameter string. Since `strlen` is an O(N) operation, this required the string to be traversed three separate times, scaling poorly for long strings and adding unnecessary CPU overhead on a logging path.
**Action:** Always compute `strlen` once and cache the result in a local `size_t` variable (e.g., `size_t len = strlen(msg)`) if the same string length is needed multiple times within a function, avoiding redundant O(N) recalculations.
