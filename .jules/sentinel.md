...
## $(date +%Y-%m-%d) - Replace unsafe strcpy calls with strlcpy in fs/real.c
**Vulnerability:** Unbounded string copies (`strcpy`) writing to fixed-size char arrays (`char name[NAME_MAX + 1]`) in `fs/real.c` when copying host directory entry names into the internal structs. If the host file system permits longer file names than `NAME_MAX`, this leads to a kernel stack/heap buffer overflow.
**Learning:** External data crossing into internal emulation (e.g., host filesystem paths) must always be explicitly bounds-checked using functions like `strlcpy`, even if standard limits are expected, because external host limits might exceed internal macros.
**Prevention:** Always use `strlcpy` or explicitly check `strlen` against `sizeof(dest)` before copying host boundary string data to internal structs to ensure strict memory isolation and prevent buffer overflow vulnerabilities.
