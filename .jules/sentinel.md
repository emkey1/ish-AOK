...
## $(date +%Y-%m-%d) - Buffer Overflow via Reversed strncpy in flock_from_file_lock
**Vulnerability:** In `fs/lock.c`, `flock_from_file_lock` reversed the source and destination arguments of `strncpy`: `strncpy(lock->comm, flock->comm, 16);`. This caused the kernel lock object's state (`lock->comm`) to be overwritten with uninitialized data from the user's `flock` struct, failing to populate the user's struct properly and creating a minor memory corruption within the kernel structure.
**Learning:** Argument order in `strncpy` and `memcpy` (dest, src) is frequently prone to typos, especially when copying between structures with similar field names, potentially corrupting kernel memory with user-provided or uninitialized data.
**Prevention:** Always verify the direction of data flow (kernel to user vs user to kernel) matches the argument order in copy functions, especially when bridging internal structures to user-facing syscall structures.
