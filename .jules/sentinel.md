...
## $(date +%Y-%m-%d) - Inverted String Copy in lock handling
**Vulnerability:** In `fs/lock.c`, `flock_from_file_lock` reversed the source and destination arguments of `strncpy`, accidentally copying from the uninitialized user-provided `flock->comm` into the kernel's `lock->comm`, leading to memory corruption of the lock structure and failure to return the lock holding process to the user.
**Learning:** Functions that translate between kernel structures (e.g., `struct file_lock`) and user-facing structures (e.g., `struct flock_`) must strictly flow data from kernel state to user state. Magic numbers (`16`) should also be avoided in favor of `sizeof()` on the destination buffer.
**Prevention:** When populating user-facing structures, ensure `strncpy` destination is the user struct and source is the kernel struct. Always use `sizeof(dest)` instead of hardcoded lengths to prevent overflows if the structure definition changes.
