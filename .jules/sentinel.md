...
## 2024-05-24 - Buffer Overflow via Unbounded String Copies at Filesystem Boundaries
**Vulnerability:** Use of unbounded `strcpy` function to copy host filesystem strings (directory names and paths) into fixed-size buffer arrays within the emulator (`entry->name` and `buf` bounded by `NAME_MAX` or `MAX_PATH`) in `fs/real.c` and `fs/aok.c`.
**Learning:** Data crossing the boundary from the host OS into the emulator must be explicitly bounds-checked or copied using bounded functions, as host limits may exceed emulator limits, potentially leading to stack/heap buffer overflows if unchecked strings are blindly copied into fixed arrays.
**Prevention:** Always use bounded string copy operations (like `strncpy` or `strlcpy`) when populating fixed-size arrays from external or unbounded sources. If using `strncpy`, ensure explicit null-termination of the final byte (`array[sizeof(array) - 1] = '\0'`).
