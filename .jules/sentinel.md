...
## 2026-09-16 - sprintf Bounds Checking Logic Error
**Vulnerability:** When replacing `sprintf` with `snprintf` in dynamic allocations (e.g. `main.c` `build_initial_envp`), using the pre-calculated string length (`term_size`) without adding the `+ 1` for the null terminator causes `snprintf` to truncate the final character of the formatted string, breaking expected logic (e.g., `envp` variables missing their last character).
**Learning:** `snprintf(buf, size, ...)` interprets `size` as the strict maximum number of bytes to write *including* the null terminator. If `size` is precisely the string length, the string itself will be truncated by one character.
**Prevention:** When using `snprintf` with dynamically allocated buffers based on precise length calculations, ensure the `size` argument includes the `+ 1` required for the null byte to prevent unintended truncation.
