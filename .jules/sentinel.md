...
## 2026-03-27 - Buffer Overflow in Path Formatting using sprintf
**Vulnerability:** The functions formatting PID-based paths (`/proc/%d/mem` and `/proc/%d/maps`) in `tools/ptutil.c` and `tools/vdso-transplant.c` used `sprintf` with statically sized buffers. While the buffers were 1024 and 32 bytes respectively, relying on `sprintf` is a security risk. If a PID representation exceeds expectations, a stack-based buffer overflow occurs.
**Learning:** Hardcoded stack buffer sizes paired with unbounded `sprintf` for formatting user or system-controlled input (like PID values into path names) represents a defense-in-depth vulnerability.
**Prevention:** Always use bounded string formatting functions like `snprintf` combined with `sizeof(buffer)` when formatting paths or incorporating externally influenced integers, eliminating any possibility of overflow regardless of buffer size.
