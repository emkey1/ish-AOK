...
## 2024-05-15 - Incorrect Privilege Check in sys_sethostname
**Vulnerability:** The `sys_sethostname` syscall checked for root privileges using `current->uid != 0` instead of the standard `superuser()` macro (which checks `euid`). This created an authorization bypass where processes with dropped effective privileges but real uid 0 could inappropriately execute the syscall, and legitimately privileged setuid root processes (euid 0, real uid != 0) were denied access.
**Learning:** Always check effective user IDs (via standard macros like `superuser()`) rather than real user IDs for capability and privilege checks to respect setuid/setgid execution and dropped privileges correctly.
**Prevention:** Use standard privilege macros (e.g., `superuser()`) exclusively, and implement linting rules to ban direct checks against `current->uid` for authorization boundaries.
