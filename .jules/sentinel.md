...
## 2024-05-24 - Buffer Overflow in PTY filesystem path formatting
**Vulnerability:** The functions `devpts_getpath` and `devpts_readdir` in `fs/pty.c` previously used the unsafe `sprintf` function to format pseudo-terminal paths into output buffers. If `devpts.num` or `pty_num` values were to grow unexpectedly large (e.g., beyond the standard size expected), this could lead to a stack buffer overflow.
**Learning:** Hardcoded path assumptions with `sprintf` present latent security risks. Values that are theoretically bounded but rely on external type lengths (like integer conversions to strings) can exceed expected buffer sizes if the data model changes.
**Prevention:** Always use bounded string formatting functions like `snprintf` when formatting integers into paths or strings within fixed-size buffers, utilizing constants like `MAX_PATH` or `sizeof()` as the limit to explicitly prevent buffer overruns.
