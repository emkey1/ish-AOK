...
void mem_destroy(struct mem *mem) {
    write_lock(&mem->lock);
    while((mem_ref_cnt_val(mem)) && (current->pid > 1) ){ 
...
static void mem_pt_del(struct mem *mem, page_t page) {
    ...
    if (entry != NULL) {
         while(task_ref_cnt_val(current) > 4) { // mark
             nanosleep(&lock_pause, NULL);
        }
        entry->data = NULL;
    }
...
int pt_unmap_always(struct mem *mem, page_t start, pages_t pages) {
    for (page_t page = start; page < start + pages; mem_next_page(mem, &page)) {
        while(task_ref_cnt_val(current) >3) {
            nanosleep(&lock_pause, NULL);
        }
...
int pt_copy_on_write(struct mem *src, struct mem *dst, page_t start, page_t pages) {
    while(task_ref_cnt_val(current)) { // Wait for now, task is in one or more critical sections
        nanosleep(&lock_pause, NULL);
    }
...
void *mem_ptr(struct mem *mem, addr_t addr, int type) {
    ...
    if (entry != NULL && (type == MEM_WRITE || type == MEM_WRITE_PTRACE)) {
        ...
        if (entry->flags & P_COW) {
            ...
            mem_ref_cnt_mod(mem, 1,__FILE__, __LINE__);
            //read_to_write_lock(&mem->lock);
